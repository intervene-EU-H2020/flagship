...
#FinnGen
finngen <- fread("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/FinnGen/HRperSD_AgeandSexStratified_FinnGen.csv", data.table=FALSE)
...
mgb <- fread("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/PartnersBiobank/HRperSD_AgeandSexStratified_MGBB.csv", data.table=FALSE)
...
colnames(metaresults) <- c("Ancestry","Quartile","Sex","Phenotype","Beta","SE","Pval","QHet","HetPval")
...
fwrite(metaresults, "/Users/jermy/Documents/INTERVENE/Results/MetaAnalysis/AgeandSexStratifiedMetaAnalysisFE.csv")

######################################################################################################################################

all <- all[,c("Ancestry","Quartile","Sex","Phenotype","Beta","SE","Biobank")]
...
# Main analysis
meta <- fread("/Users/jermy/Documents/INTERVENE/Results/MetaAnalysis/AgeandSexStratifiedMetaAnalysisFE.csv", data.table=FALSE)
meta$Ancestry <- factor(meta$Ancestry, levels=c("EUR","SAS","EAS"))
orderPheno <- subset(meta, Ancestry=="EUR" & Quartile==1 & Sex=="female")
...
europelabels <- list(c("Appendicitis","ILD","Lung Cancer","All Cancers","Major Depression","Hip Osteoarthritis","Epilepsy","Skin Melanoma","Knee Osteoarthritis","CHD","Colorectal Cancer","Asthma","Gout","Atrial Fibrillation","Type 2 Diabetes","Type 1 Diabetes"),
                     c("Appendicitis","ILD","All Cancers","Major Depression","Hip Osteoarthritis","Epilepsy","Knee Osteoarthritis","CHD","Asthma","Gout","Atrial Fibrillation","Type 2 Diabetes"),
                     c("All Cancers","Epilepsy","CHD","Colorectal Cancer","Asthma","Type 2 Diabetes")
)
...
for(i in 1:3){
  ...
  #Plot
  print(ggplot(anc) +
          geom_point(aes(Phenotype, HR, group=interaction(Quartile,Sex), col=Quartile, shape=Sex), position=position_dodge(width=0.7)) +
          theme_bw() +
          geom_errorbar(aes(x=Phenotype, ymin = Cineg, ymax = Cipos, group=interaction(Quartile,Sex), col=Quartile), size=0.5, width=0.5, position=position_dodge(width=0.7)) +
          ...
          coord_flip())
  ggsave(paste0("/Users/jermy/Documents/INTERVENE/Write-up/Figure4_",ancestry[i],".png"), height=8, width=8)
...
for(k in c(1,2,3,4)){
  for(l in c("male","female")){
    ...
    print(ggplot(age) +
      ...
      xlab("") +
      geom_hline(yintercept = 1.0) +
      scale_x_discrete(labels=c("Appendicitis","ILD","Lung Cancer","All Cancers","Major Depression","Hip Osteoarthritis","Epilepsy","Skin Melanoma","Knee Osteoarthritis","CHD","Colorectal Cancer","Asthma","Gout","Atrial Fibrillation","Type 2 Diabetes","Type 1 Diabetes")) + 
