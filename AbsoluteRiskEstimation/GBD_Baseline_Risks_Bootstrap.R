...
nk<-3
countries<-c("Massachusetts","Norway","Estonia","Finland","England","United Kingdom","Japan","Scotland")
result<-c()
for(j in 1:length(gbd_phenos)){
  for (c in 1:length(countries)){
    ...
    incidence$mortandrisk <- cumsum(incidence$hazard + incidence$mortality_rate)
    incidence$survival <- exp(-5*incidence$mortandrisk)
    
    #Calculate lifetime risk as the cumulative sum of the product of survival and risk.
    incidence$lifetimerisk <- cumsum(incidence$survival*incidence$risk)*100
    ...
    incidence$cumulativerisk <- cumsum(incidence$hazard)
    #subby$survival_noncomp <- exp(-5*subby$cumulativerisk)
    
    #Calculate lifetime risk as the cumulative sum of the product of survival and risk.
    #subby$lifetimerisk_noncomp <- cumsum(subby$survival_noncomp*subby$risk)*100
    
    result<-rbind(result,incidence)
  }
}
fwrite(result,paste0(output_dir,"GBD_LifetimeRisks.csv"),quote=FALSE,row.names=FALSE,sep=",")
...
############################# PLOTTING #############################
incidence<-fread(paste0(output_dir,"GBD_LifetimeRisks.csv"),fill=TRUE)
