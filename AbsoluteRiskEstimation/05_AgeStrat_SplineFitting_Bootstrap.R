...
filename <- c("/HR_AgeStratified_MGBB_EUR.csv")
...
for(l in 1:length(biobank)){
  ...
  for(j in 1:length(phenocols)){
    ...
    actual <- fread(paste0("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/",biobank[l],"/Age/",phenocols[j],"_Age_Specific_Hazards.csv"), select=c("Age","Beta","Group"), data.table=FALSE)
    bootstrapped_results <- inner_join(bootstrapped_results, actual)
    
    write.csv(bootstrapped_results, paste0("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/",biobank[l],"/Age/",phenocols[j],"_Age_Specific_Hazards_Bootstrapped.csv"))
...
for(l in 1:length(biobank)){
  ...
  for(j in 1:length(phenocols)){
    ...
    #Add in actual lifetime risks
    actual <- fread(paste0("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/",biobank[l],"/Age/",phenocols[j],"_Age_Specific_Hazards_OnePercentTails.csv"), select=c("Age","Beta","Group"), data.table=FALSE)
    bootstrapped_results <- inner_join(bootstrapped_results, actual)
    
    write.csv(bootstrapped_results, paste0("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/",biobank[l],"/Age/",phenocols[j],"_Age_Specific_Hazards_Bootstrapped_OnePercentTails.csv"))
