...
#countries <- c("Finland","Massachusetts","Norway","United Kingdom","Estonia", "United Kingdom", "United Kingdom")
#biobank <- c("FinnGen", "PartnersBiobank", "HUNT", "GenomicsEngland", "EstonianBiobank", "UKBiobank", "UKMetaAnalysis")
...
for(j in 1:length(biobank)){
  ...
  for(k in 1){
    ...
    #file <- c(paste0("/R10_HR_",sexes[k],"Sample_FinnGen.csv"), paste0("/HR_",sexes[k],"Sample_MGBB_EUR.csv"), paste0("/HR_",sexes[k],"Sample_HUNT.csv"), paste0("/HR_",sexes[k],"Sample_GenomicsEngland.csv"), paste0("/HR_",sexes[k],"Sample_EstBB.csv"), paste0("/HR_",sexes[k],"Sample_UKB_AllAncestries.csv"), paste0("/UKMetaAnalysis",sexes[k],"Sample.csv"))
    
    file <- c(paste0("/HR_",sexes[k],"Sample_MGBB_EUR.csv"))
    ...
    while(m < 1000){
      ...
      hazrats <- fread(paste0("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/",biobank[j],file[j]), data.table = FALSE)
      
      if(biobank[j] == "UKMetaAnalysis"){
        colnames(hazrats) <- c("Phenotype", "Sex", "Group", "Beta", "SE", "Pval", "QHet", "HetPval", "HR", "CIpos", "CIneg")
      }else if(biobank[j]=="UKBiobank"){
        hazrats <- hazrats[,-1]
        hazrats <- hazrats
      }else{
      ...
      hazrats <- subset(hazrats, Phenotype==hr_phenos)
...
for(j in 1:length(biobank)){
  ...
  for(k in 1){
    ...
    file <- c(paste0("/R10_HR_",sexes[k],"Sample_FinnGen.csv"))
    ...
    while(m < 1000){
      ...
      #Hazard Ratios
      hr01 <- with(hazrats[hazrats$Group=="< 5%",], exp(rnorm(nrow(hazrats[hazrats$Group=="< 5%",]), hazrats[hazrats$Group=="< 5%","Beta"], hazrats[hazrats$Group=="< 5%","SE"])))
      hr02 <- with(hazrats[hazrats$Group=="5-10%",], exp(rnorm(nrow(hazrats[hazrats$Group=="5-10%",]), hazrats[hazrats$Group=="5-10%","Beta"], hazrats[hazrats$Group=="5-10%","SE"])))
      hr03 <- with(hazrats[hazrats$Group=="10-20%",], exp(rnorm(nrow(hazrats[hazrats$Group=="10-20%",]), hazrats[hazrats$Group=="10-20%","Beta"], hazrats[hazrats$Group=="10-20%","SE"])))
      hr04 <- with(hazrats[hazrats$Group=="20-40%",], exp(rnorm(nrow(hazrats[hazrats$Group=="20-40%",]), hazrats[hazrats$Group=="20-40%","Beta"], hazrats[hazrats$Group=="20-40%","SE"])))
      hr06 <- with(hazrats[hazrats$Group=="60-80%",], exp(rnorm(nrow(hazrats[hazrats$Group=="60-80%",]), hazrats[hazrats$Group=="60-80%","Beta"], hazrats[hazrats$Group=="60-80%","SE"])))
      hr07 <- with(hazrats[hazrats$Group=="80-90%",], exp(rnorm(nrow(hazrats[hazrats$Group=="80-90%",]), hazrats[hazrats$Group=="80-90%","Beta"], hazrats[hazrats$Group=="80-90%","SE"])))
      hr08 <- with(hazrats[hazrats$Group=="90-95%",], exp(rnorm(nrow(hazrats[hazrats$Group=="90-95%",]), hazrats[hazrats$Group=="90-95%","Beta"], hazrats[hazrats$Group=="90-95%","SE"])))
      hr09 <- with(hazrats[hazrats$Group=="95-99%",], exp(rnorm(nrow(hazrats[hazrats$Group=="95-99%",]), hazrats[hazrats$Group=="95-99%","Beta"], hazrats[hazrats$Group=="95-99%","SE"])))
      hr10 <- with(hazrats[hazrats$Group=="> 99%",], exp(rnorm(nrow(hazrats[hazrats$Group=="> 99%",]), hazrats[hazrats$Group=="> 99%","Beta"], hazrats[hazrats$Group=="> 99%","SE"])))
