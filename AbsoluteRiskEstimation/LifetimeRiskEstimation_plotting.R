...
results_dir<-"/mnt/work/workbench/bwolford/intervene/results/" #set this to your directory of choice
...
riskwithintervals <- subset(results, Group=="Group1" | Group=="Group6" | Group=="Group11")
pdf(file="Risks_Facet.pdf",height=10,width=10,useDingbats=TRUE)
...
ggplot(riskwithintervals, aes(Age, LifetimeRisk, fill=Group, color=Group, group=Group)) +
  ...
  theme_bw() + facet_wrap(~biobank+trait)
