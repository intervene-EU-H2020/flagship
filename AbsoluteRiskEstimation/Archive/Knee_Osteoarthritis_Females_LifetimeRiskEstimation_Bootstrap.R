...
gbd_phenos <- c("Osteoarthritis knee")
hr_phenos <- c("KNEE_ARTHROSIS")
directory <- c("KneeOsteoarthritis")
sexes <- c("Female")

#countries <- c("Finland","Massachusetts","Norway","United Kingdom","Scotland","Estonia","United Kingdom")
...
for(j in 1:length(biobank)){
  ...
  for(k in 1){
    ...
    while(m < 1000){
      ...
      hazrats <- fread(paste0("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/",biobank[j],"/AgeandSex/",hr_phenos,"_",tolower(sexes[k]),"_Age_Specific_Hazards_Bootstrapped.csv"), data.table = FALSE)
      hazrats <- hazrats[,-1]
    ...
    lifetimeriskactual <- fread(paste0("/Users/jermy/Documents/INTERVENE/Results/GBD_Incidence/",biobank[j],"/",directory,"/",hr_phenos,"_",sexes[k],"LifetimeRisk_",biobank[j],".csv"), select=c("LifetimeRisk"), data.table=FALSE)
    ...
    write.csv(bootstrapped_lifetimerisk, paste0("/Users/jermy/Documents/INTERVENE/Results/GBD_Incidence/",biobank[j],"/",directory,"/",hr_phenos,"_",sexes[k],"LifetimeRisk_Bootstrapped_",biobank[j],".csv"))
    ...
    ggsave(paste0("/Users/jermy/Documents/INTERVENE/Results/GBD_Incidence/",biobank[j],"/",directory,"/",hr_phenos,"_",sexes[k],"LifetimeRisk_Bootstrapped_",biobank[j],".png"), height=10 , width=10)
...
for(j in 1:length(biobank)){
  ...
  for(k in 1){
    ...
    lifetimerisks <- data.frame(Age=rep(c("1-4 years","5-9 years","10-14 years","15-19 years","20-24 years","25-29 years","30-34 years","35-39 years","40-44 years","45-49 years","50-54 years","55-59 years","60-64 years","65-69 years","70-74 years","75-79 years"),11),
                                Group=c(rep("< 1%",16), rep("1-5%",16), rep("5-10%",16), rep("10-20%",16), rep("20-40%",16), rep("40-60%",16), rep("60-80%",16), rep("80-90%",16), rep("90-95%",16), rep("95-99%",16), rep("> 99%",16)))
    ...
    while(m < 1000){
      ...
      #Read in the hazard ratios and allocate to variables...
      hazrats <- fread(paste0("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/",biobank[j],"/AgeandSex/",hr_phenos,"_",tolower(sexes[k]),"_Age_Specific_Hazards_Bootstrapped_OnePercentTails.csv"), data.table = FALSE)
      hazrats <- hazrats[,-1]
      hazrats$sdpos <- hazrats$CIpos - hazrats$Beta 
      hazrats$sdneg <- hazrats$Beta - hazrats$CIneg
      hazrats$SD <- rowMeans(hazrats[,c("sdpos","sdneg")], na.rm=T)
      ...
      #Hazard Ratios
      hr01 <- with(hazrats[hazrats$Group=="< 1%",], exp(rnorm(nrow(hazrats[hazrats$Group=="< 1%",]), hazrats[hazrats$Group=="< 1%","Beta"], hazrats[hazrats$Group=="< 1%","SD"])))
      hr02 <- with(hazrats[hazrats$Group=="1-5%",], exp(rnorm(nrow(hazrats[hazrats$Group=="1-5%",]), hazrats[hazrats$Group=="1-5%","Beta"], hazrats[hazrats$Group=="1-5%","SD"])))
      hr03 <- with(hazrats[hazrats$Group=="5-10%",], exp(rnorm(nrow(hazrats[hazrats$Group=="5-10%",]), hazrats[hazrats$Group=="5-10%","Beta"], hazrats[hazrats$Group=="5-10%","SD"])))
      hr04 <- with(hazrats[hazrats$Group=="10-20%",], exp(rnorm(nrow(hazrats[hazrats$Group=="10-20%",]), hazrats[hazrats$Group=="10-20%","Beta"], hazrats[hazrats$Group=="10-20%","SD"])))
      hr05 <- with(hazrats[hazrats$Group=="20-40%",], exp(rnorm(nrow(hazrats[hazrats$Group=="20-40%",]), hazrats[hazrats$Group=="20-40%","Beta"], hazrats[hazrats$Group=="20-40%","SD"])))
      hr07 <- with(hazrats[hazrats$Group=="60-80%",], exp(rnorm(nrow(hazrats[hazrats$Group=="60-80%",]), hazrats[hazrats$Group=="60-80%","Beta"], hazrats[hazrats$Group=="60-80%","SD"])))
      hr08 <- with(hazrats[hazrats$Group=="80-90%",], exp(rnorm(nrow(hazrats[hazrats$Group=="80-90%",]), hazrats[hazrats$Group=="80-90%","Beta"], hazrats[hazrats$Group=="80-90%","SD"])))
      hr09 <- with(hazrats[hazrats$Group=="90-95%",], exp(rnorm(nrow(hazrats[hazrats$Group=="90-95%",]), hazrats[hazrats$Group=="90-95%","Beta"], hazrats[hazrats$Group=="90-95%","SD"])))
      hr10 <- with(hazrats[hazrats$Group=="95-99%",], exp(rnorm(nrow(hazrats[hazrats$Group=="95-99%",]), hazrats[hazrats$Group=="95-99%","Beta"], hazrats[hazrats$Group=="95-99%","SD"])))
      hr11 <- with(hazrats[hazrats$Group=="> 99%",], exp(rnorm(nrow(hazrats[hazrats$Group=="> 99%",]), hazrats[hazrats$Group=="> 99%","Beta"], hazrats[hazrats$Group=="> 99%","SD"])))
      ...
      Groups <- c("< 1%", "1-5%", "5-10%", "10-20%", "20-40%", "40-60%", "60-80%", "80-90%", "90-95%", "95-99%", "> 99%")
      lifetimerisk <- data.frame(NULL)
      for(i in 1:11){
        #Calculate hazard
        incidence[[paste0("hazard",i)]] <- incidence[[paste0("i",i)]] / (1 - incidence$prevalence)
    ...
    bootstrapped_lifetimerisk <- data.frame(Age=rep(c("1-4 years","5-9 years","10-14 years","15-19 years","20-24 years","25-29 years","30-34 years","35-39 years","40-44 years","45-49 years","50-54 years","55-59 years","60-64 years","65-69 years","70-74 years","75-79 years"),11),
                                            Group=c(rep("< 1%",16), rep("1-5%",16), rep("5-10%",16), rep("10-20%",16), rep("20-40%",16), rep("40-60%",16), rep("60-80%",16), rep("80-90%",16), rep("90-95%",16), rep("95-99%",16), rep("> 99%",16)))
    ...
    #Add in actual lifetime risks
    lifetimeriskactual <- fread(paste0("/Users/jermy/Documents/INTERVENE/Results/GBD_Incidence/",biobank[j],"/",directory,"/",hr_phenos,"_",sexes[k],"LifetimeRisk_",biobank[j],"_OnePercentTails.csv"), select=c("LifetimeRisk"), data.table=FALSE)
    bootstrapped_lifetimerisk <- cbind(bootstrapped_lifetimerisk, lifetimeriskactual)
    
    #Plot all as well as overall lifetime risk
    bootstrapped_lifetimerisk$Age <- factor(bootstrapped_lifetimerisk$Age, levels=c("1-4 years","5-9 years","10-14 years","15-19 years","20-24 years","25-29 years","30-34 years","35-39 years","40-44 years","45-49 years","50-54 years","55-59 years","60-64 years","65-69 years","70-74 years","75-79 years"))
    bootstrapped_lifetimerisk$age <- rep(c(5,10,15,20,25,30,35,40,45,50,55,60,65,70,75,80),11)
    bootstrapped_lifetimerisk$Group <- factor(bootstrapped_lifetimerisk$Group, levels=c(c("< 1%", "1-5%", "5-10%", "10-20%", "20-40%", "40-60%", "60-80%", "80-90%", "90-95%", "95-99%", "> 99%")))
    
    write.csv(bootstrapped_lifetimerisk, paste0("/Users/jermy/Documents/INTERVENE/Results/GBD_Incidence/",biobank[j],"/",directory,"/",hr_phenos,"_",sexes[k],"LifetimeRisk_Bootstrapped_",biobank[j],"_OnePercentTails.csv"))
    
    #Considering confidence intervals
    riskwithintervals <- subset(bootstrapped_lifetimerisk, Group=="< 1%" | Group=="40-60%" | Group=="> 99%")
    ...
    ggplot(riskwithintervals, aes(age, LifetimeRisk, color=Group, group=Group)) +
      ...
      theme_bw() +
      labs(color='PRS Group', fill='PRS Group') +
      scale_color_hue(labels = c("< 1%", "40-60%", "> 99%")) +
      scale_fill_hue(labels = c("< 1%", "40-60%", "> 99%")) +
      theme(title = element_text(size = 22),
            legend.text = element_text(size = 16),
            legend.title = element_text(size = 18),
            axis.title.x = element_text(size = 18),
            ...
            axis.title.y = element_text(size = 18),
            axis.text.y = element_text(size = 16))
    ggsave(paste0("/Users/jermy/Documents/INTERVENE/Results/GBD_Incidence/",biobank[j],"/",directory,"/",hr_phenos,"_",sexes[k],"LifetimeRisk_Bootstrapped_",biobank[j],"_OnePercentTails.png"), height=10 , width=10)
