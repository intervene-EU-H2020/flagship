...
for(j in 1:length(biobank)){
  ...
  for(k in 1){
    ...
    hazrats <- fread(paste0("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/",biobank[j],"/AgeandSex/",hr_phenos,"_",tolower(sexes[k]),"_Age_Specific_Hazards.csv"), data.table = FALSE)
    hazrats <- hazrats[,-1]
    ...
    write.csv(lifetimerisk, paste0("/Users/jermy/Documents/INTERVENE/Results/GBD_Incidence/",biobank[j],"/",directory,"/",hr_phenos,"_",sexes[k],"LifetimeRisk_",biobank[j],".csv"))
    ...
    ggsave(paste0("/Users/jermy/Documents/INTERVENE/Results/GBD_Incidence/",biobank[j],"/",directory,"/",hr_phenos,"_",sexes[k],"LifetimeRisk_",biobank[j],".png"), height=6, width=8, dpi=300)
...
for(j in 1:length(biobank)){
  ...
  for(k in 1){
    ...
    #Read in the hazard ratios and allocate to variables...
    hazrats <- fread(paste0("/Users/jermy/Documents/INTERVENE/Results/HazardRatios/",biobank[j],"/AgeandSex/",hr_phenos,"_",tolower(sexes[k]),"_Age_Specific_Hazards_OnePercentTails.csv"), data.table = FALSE)
    hazrats <- hazrats[,-1]
    colnames(hazrats) <- c("Age","Beta","HR","Group")
    ...
    #Plot all as well as overall lifetime risk
    lifetimerisk$Age <- factor(lifetimerisk$Age, levels=c("1-4 years","5-9 years","10-14 years","15-19 years","20-24 years","25-29 years","30-34 years","35-39 years","40-44 years","45-49 years","50-54 years","55-59 years","60-64 years","65-69 years","70-74 years","75-79 years"))
    lifetimerisk$age <- rep(c(5,10,15,20,25,30,35,40,45,50,55,60,65,70,75,80),11)
    lifetimerisk$Group <- factor(lifetimerisk$Group, levels=c("> 99%", "95-99%", "90-95%", "80-90%", "60-80%", "40-60%", "20-40%", "10-20%", "5-10%", "1-5%", "< 1%"))
    
    write.csv(lifetimerisk, paste0("/Users/jermy/Documents/INTERVENE/Results/GBD_Incidence/",biobank[j],"/",directory,"/",hr_phenos,"_",sexes[k],"LifetimeRisk_",biobank[j],"_OnePercentTails.csv"))
    ...
    ggplot(lifetimerisk, aes(age, LifetimeRisk, color=Group, group=Group)) +
      ...
      theme(legend.text = element_text(size = 24),
            ...
            axis.title.y = element_text(size = 28),
            axis.text.y = element_text(size = 24))
    ggsave(paste0("/Users/jermy/Documents/INTERVENE/Results/GBD_Incidence/",biobank[j],"/",directory,"/",hr_phenos,"_",sexes[k],"LifetimeRisk_",biobank[j],"_OnePercentTails.png"), height=6, width=8, dpi=300)
