...
gbd_phenos <- c("Malignant skin melanoma")
hr_phenos <- c("C3_MELANOMA_SKIN")
directory <- c("Melanoma")
sexes <- c("Male", "Female")
...
for(j in 1:length(biobank)){
  ...
  for(k in 1:2){
    ...
    #file <- c(paste0("/R10_HR_FullSample_FinnGen.csv"), paste0("/HR_FullSampleMGBB_EUR.csv"), paste0("/HR_FullSample_HUNT.csv"), paste0("/HR_FullSample_Genomics_England.csv"), paste0("/HR_FullSample_GS.csv"), paste0("/HR_FullSample_EstBB.csv"), paste0("/UKMetaAnalysisPercentilesFullSample.csv"))
    
    file <- c(paste0("/HR_FullSampleMGBB_EUR.csv"))
    ...
    while(m < 1000){
      ...
      if(biobank[j] == "UKMetaAnalysis"){
        colnames(hazrats) <- c("Phenotype", "Group", "Beta", "SE", "Pval", "QHet", "HetPval", "HR", "CIpos", "CIneg")
      } else {
...
for(j in 1:length(biobank)){
  ...
  for(k in 1:2){
    ...
    file <- c(paste0("/R10_HR_FullSample_FinnGen.csv"))
