...
for(i in 1:length(phenocols)){
  ...
  pheno <- fread(input="path/to/pheno_file", select=c("ID","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10",phenocols[i],paste0(phenocols[i],"_DATE"),"END_OF_FOLLOWUP","SEX"), data.table=FALSE)
  ...
  for(k in c("male","female")){
    ...
    for(j in c(1,2,3,4)){
      ...
      survival <- coxph(as.formula(paste0("Surv(tstart, AGE, event) ~ ",prscols[i],"_prs + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")), data=pheno_split_sub, na.action=na.exclude, control=coxph.control(iter.max=100))
...
write.csv(results, "file/path/to/output/HRperSD_AgeandSexStratified_[ENTER_BIOBANK_NAME].csv")